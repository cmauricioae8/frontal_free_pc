...
class PcPubChecker : public rclcpp::Node
{
  public:
    PcPubChecker() : Node(node_name+"_checker")
    {
        cb_group_ = this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
        timer_ = this->create_wall_timer(1000ms, std::bind(&PcPubChecker::timer_function, this), cb_group_);
    }
  ...
  private:
    void timer_function()
    {
        ...
        if( current_time - last_cloud_time > 5){
...
class FrontalFreePC : public rclcpp::Node
{
    ...
    void pointcloud2_CB(const sensor_msgs::msg::PointCloud2::SharedPtr cloud_msg) {
        ...
        cb_hz_ = 1/(cb_time_ - last_cloud_time);
        last_cloud_time = cb_time_;
...
int main(int argc, char * argv[])
{
    ...
    auto pc_pub_checker_node = std::make_shared<PcPubChecker>();

    rclcpp::executors::MultiThreadedExecutor executor;
    executor.add_node(frontal_free_pc_node);
    executor.add_node(pc_pub_checker_node);
    executor.spin();
